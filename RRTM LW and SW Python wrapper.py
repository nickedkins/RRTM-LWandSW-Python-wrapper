...
def writeformattedcloudfile():
	...
	f.write('   {:2d}    {:1d}    {:1d}\n'.format(inflags_master[i_zon,i_lat],iceflags_master[i_zon,i_lat],liqflags_master[i_zon,i_lat]))
...
def convection(T,z,conv_log):
	...
	for i in range(1,len(T)):
		dT = (T[i]-T[i-1])
		dz = (z[i]-z[i-1])/1000.
		if( (-1.0 * dT/dz > lapse or z[i]/1000. < 0.) and z[i]/1000. < 20. ):
			if(conv_log==1):
				conv[i]=1.
			T[i] = T[i-1] - lapse * dz
...
c_zonal=4.0 #zonal transport coefficient
# c_merid=0.7 #meridional transport coefficient
		...
		for i_tbound in range(len(tbounds)):
			...
			for wklfac in wklfacs:
				...
				if(input_source==1):
					...
					# inflags=np.ones(nzoncols)
					# iceflags=np.ones(nzoncols)
					# liqflags=np.ones(nzoncols)
					# cld_lays=np.ones(nzoncols)
					# cld_fracs=np.ones(nzoncols)
					# tauclds=np.ones((nzoncols,nlatcols))
					# ssaclds=np.ones((nzoncols,nlatcols))
					...
					vars_master_zon_lat=[inflags_master,iceflags_master,liqflags_master,tbound_master,toa_fnet_master,zonal_transps,merid_transps]
				...
				for ts in range(timesteps):
					...
					for i_lat in range(nlatcols):
						...
						for i_zon in range(nzoncols):
							...
							cti=0
							for i in range(len(conv_master[:,i_zon,i_lat])):
								if(conv_master[i,i_zon,i_lat]==1):
									cti=i
								else:
									continue
							...
							column_budgets_master[i_zon,i_lat]=toa_fnet+merid_transps_master[i_zon,i_lat]+extra_forcing  #nje forcing
					...
					for i_lat in range(nlatcols):
						for i_zon in range(nzoncols):
							for i in range(nlayers):
								dfnet_master[i,i_zon,i_lat]=fnet_master[i+1,i_zon,i_lat]-fnet_master[i,i_zon,i_lat]
