...
prev_output_file='/Users/nickedkins/Dropbox/GitHub Repositories/RRTM-LWandSW-Python-wrapper/_Useful Data/PRP to eqb/_baseline_v4.1'
												...
												for ts in range(timesteps):
													...
													for i_lat in range(nlatcols):
														...
														for i_zon in range(nzoncols):
															...
															if((input_source==0 and ts>50) or input_source==2):
																# dtbound=toa_fnet*0.1*0.5*0.1
																dtbound=column_budgets_master[i_zon,i_lat]*0.04
																if(input_source==2):
																	dtbound*=2.
																dtbound=np.clip(dtbound,-dmax,dmax)
																tbound+=dtbound
															...
															if((sw_on==1 and ts%500==1) or ((input_source==1 or input_source==1) and ts==1)):
													...
													if(input_source==0 or input_source==2):
														for i_lat in range(nlatcols):
															for i_zon in range(nzoncols):
																for i in range(nlayers):
																	cldweights=[0.5,0.5]
																	dT=np.mean(dfnet_master[i,:,i_lat]/dpz_master[i,:,i_lat]*cldweights)*-1.*2.*undrelax_lats[i_lat]*(60./nlayers)*4.
																	if(input_source==2):
																		dT*=10.
													...
													if(abs(maxdfnet_tot) < eqb_maxdfnet and (ts>100 or input_source==2) and np.max(abs(column_budgets_master))<eqb_col_budgs):
														if(plotted!=1):
															plotrrtmoutput()
															plotted=1
