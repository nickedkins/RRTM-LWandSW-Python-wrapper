...
nbs=[1,2,4]
adv_on=1 # 0: no advective heating, 1: advective heating on
...
i_loops=0
totloops=np.float(len(pertzons)*len(pertlats)*len(pertmols)*len(pertlays)*len(perts)*len(c_merids)*len(c_zonals)*len(adv_locs)*len(nbs))
														...
														for ts in range(timesteps):
															...
															for i_lat in range(nlatcols):
																...
																for i_zon in range(nzoncols):
																	...
																	if((sw_on==1 and ts%300==1) or ((input_source==1 or input_source==1) and ts==1)):
