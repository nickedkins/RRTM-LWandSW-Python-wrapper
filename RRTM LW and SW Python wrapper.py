...
def read_misr():
    ...
    binned_cf = np.zeros((nlatcols,ncloudcols))
    cf_bincounts = np.zeros((nlatcols,ncloudcols))
        ...
        for wklfac in wklfacs:
            ...
            for ts in range(timesteps):
                ...
                for i_lat in range(nlatcols):
                    ...
                    for i_cld in range(ncloudcols):
                        ...
                        if(ts>1 or input_source==1):
                            ...
                            toa_fnet=toa_fnet_master[i_cld,i_lat]
                        ...
                        vars_misc_1d_lens=[16,29,29]
                        vars_master_lay_cld_nmol_lat=[wkl_master]
                        vars_master_cld_lat=[inflags_master,iceflags_master,liqflags_master,cld_lays_master,cld_fracs_master,tauclds_master,ssaclds_master,tbound_master,toa_fnet_master,zonal_transps_master]
                        ...
                        if(input_source==0 and ts>1):
                            # dtbound=toa_fnet*0.1*0.5*0.1
                            dtbound=column_budgets_master[i_cld,i_lat]*0.1*0.5
                            dtbound=np.clip(dtbound,-dmax,dmax)
                            tbound+=dtbound
                        ...
                        if(i_lat>0 and i_lat<nlatcols-1):
                            zonal_transps_master[0,i_lat]=c_merid*(tz_master[0,0,i_lat]-tz_master[0,0,i_lat+1]) + c_merid*(tz_master[0,0,i_lat-1]-tz_master[0,0,i_lat])
                        elif(i_lat==0):
                            zonal_transps_master[0,i_lat]=c_merid*(tz_master[0,0,i_lat]-tz_master[0,0,i_lat+1])
                        elif(i_lat==nlatcols-1):
                            zonal_transps_master[0,i_lat]=c_merid*(tz_master[0,0,i_lat-1]-tz_master[0,0,i_lat])


                        column_budgets_master[i_cld,i_lat]=toa_fnet+zonal_transps_master[0,i_lat]
                        ...
                        inflags_master[i_cld,i_lat]=inflags[i_cld]
                        iceflags_master[i_cld,i_lat]=iceflags[i_cld]
                        liqflags_master[i_cld,i_lat]=liqflags[i_cld]
                        cld_lays_master[i_cld,i_lat]=cld_lays[i_cld]
                        cld_fracs_master[i_cld,i_lat]=cld_fracs[i_cld]
                        tauclds_master[i_cld,i_lat]=tauclds[i_cld]
                        ssaclds_master[i_cld,i_lat]=ssaclds[i_cld]
                        # zonal_transps_master[i_cld,i_lat]=zonal_transps[i_cld]
                ...
                if(ts%10==0):
                    ...
                    for i_lat in range(nlatcols):
                        if(i_lat<nlatcols-1):
                            print '{:4.2f} {:4.2f} {:4.2f}|'.format(maxdfnet_lat[i_lat],np.mean(tbound_master[:,i_lat],axis=0),np.mean(column_budgets_master[:,i_lat],axis=0)),
                        else:
                            print '{:4.2f} {:4.2f} {:4.2f}|'.format(maxdfnet_lat[i_lat],np.mean(tbound_master[:,i_lat],axis=0),np.mean(column_budgets_master[:,i_lat],axis=0))
