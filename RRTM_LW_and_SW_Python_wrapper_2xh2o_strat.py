...
project_dir = '/Volumes/NickSD/RRTM-LWandSW-Python-wrapper/'
verbosity = 0
...
def writeoutputfile_masters():
    ...
    f = open(project_dir+'_Raw_Output_Data/'+tlabelstr,'w+')
...
def createlatdistbn(filename):
    fileloc = project_dir+'/Latitudinal_Distributions/'+filename+'.txt'
    file = open(fileloc,'r')
    lat = []
    var = []
    with file as f:
...
nlatcols=29 # number of latitude columns
...
eqb_col_budgs=1.0 # max equilibrium value of total column energy budget at TOA
...
adv_loc=0
nb=2 # power for power law scaling of z distbn of heating from horizontal transport (from Cronin and Jansen, “Analytic Radiative-Advective Equilibrium as a Model for High-Latitude Climate.”)
...
pert_strat_only = 1 # 0: use normal pert system, 1: pert only strat
...
for cf_tot in cf_tots:
                ...
                #########################################################################################################################################################
                                                            ...
                                                            if(lapse_source==0):
                                                                ...
                                                                for i_zon in range(nzoncols):
                                                                    lapse_master[i_zon,:]=np.array(createlatdistbn('Doug_Mason_Lapse_Rate_vs_Latitude'))
                                                                lapse_master*=-1.
                                                            ...
                                                            data=np.genfromtxt(project_dir+'Latitudinal_Distributions/HV19 inferred tbound.txt',delimiter=',')
                                                            lat_obs=data[:,0]
                                                            tg_obs=data[:,1]
    ...
    #                                                            adv       cld_fracs_master[i_zon,:,:],altbins,tauclds_master[0,:,:]=read_misr_2()
    #                                                                   if(ts==1):
                                                                ...
                                                                if(abs(maxdfnet_tot) < eqb_maxdfnet and (ts>200 or (input_source==2 and ts > 200)) and np.max(abs(column_budgets_master))<eqb_col_budgs):
